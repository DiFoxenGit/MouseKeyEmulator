...
1. Скачайте **`MouseKeyEmulator.exe`** из [**Releases**](../../releases/latest)
   и запустите на **обоих** компьютерах. При старте появится запрос прав
   администратора (UAC) — это нужно, чтобы управление работало во всех окнах и
   не пропадало после сворачивания; согласитесь.
...
- Подключение защищено **кодом комнаты** и **общим ключом**. Ключ проверяется
  по схеме **запрос-ответ HMAC-SHA256**: сервер присылает случайный nonce,
  клиент доказывает знание ключа, не передавая его. Перехват рукопожатия
  бесполезен, а повтор (replay) невозможен — на каждое подключение свой nonce.
- 🔑 **Задайте свой сильный общий ключ** в «Настройках» на обоих ПК (по
  умолчанию — общеизвестный, годится только для проб). Именно ключ защищает
  сопряжение: без него подключиться к комнате нельзя.
- Управляемый ПК сам решает, показывать ли экран и разрешать ли управление.
- ⚠️ Сам поток (экран/ввод) через релей по умолчанию **не шифрован** на уровне
  сервера — используйте для своих машин; для конфиденциальности разверните
  релей за TLS (домен + обратный прокси). Свой релей-сервер — в приватном
  бэкенде.
