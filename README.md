...
| | 🔌 Аппаратный (ESP32-S2) | 📶 Программный (Wi-Fi) |
|---|---|---|
| **Что нужно** | Плата ESP32-S2 в USB | Только эта же программа |
| **Как виден ввод** | Настоящее USB-HID устройство | Программный ввод (`SendInput`) |
...
> 🖥️ **Встроенный просмотр экрана** (только программный режим): на карточке
> устройства есть кнопка «Экран» — управляющий ПК показывает рабочий стол
> ПК-приёмника прямо в окне приложения, без Parsec. Приёмник может отключить
> это тумблером «Разрешить просмотр экрана». Нужен открытый TCP-порт **45124**
> на приёмнике. (Для ESP32-платы просмотр недоступен — у платы нет экрана.)
