...
В шапке три роли: **Управление**, **Приём Wi-Fi**, **Экран ESP**.

| | 🔌 Локально (ESP32) | 📶 По Wi-Fi |
|---|---|---|
| **Ввод (мышь/клава)** | Через плату ESP32-S2 (настоящее USB-HID) | Программно, `SendInput` |
| **Экран** | Стрим между двумя копиями программы (роль «Экран ESP») | Тем же приложением (роль «Приём Wi-Fi») |
| **Роль на управляемом ПК** | «Экран ESP» — только отдаёт экран, ввод идёт с платы | «Приём Wi-Fi» — ввод **и** экран |
| **Когда выбрать** | Нужна неотличимость от «железа» | Быстро, без оборудования |

На управляющем ПК в обоих случаях — роль **«Управление»**: захват мыши/клавиатуры и кнопка **«Экран»** на карточке устройства для встроенного просмотра.
